...
### Evaluation Metric
I used a metric similar to Jaccard similarity, which evaluates the similarity of two sets. Given two sets A and B, the Jaccard similarity is defined as the length of their intersection divided by the length of their union: | A n B | / | A u B |

Instead of using their union in the denominator, I take the minimum length of sets A and B: | A n B | / min(|A|, |B|). This variation focuses more on the generated content. More importantly, it minimizes the penalty from length discrepancies between actual and generated infobox lengths. 
